...
let milliseconds = [200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500];
let breakAudios = milliseconds.map(time => ({time, url: `https;//xxx.xx/silence${time}.mp3`}));

describe("ClovaSSML", () => {
  describe("#convert", () => {
    it("SimpleSpeech", function() {
      let converter = new ClovaSSML(breakAudios, 'ja');
      let ssml = '<speak>Hello! <prosody rate="120%">Clova.</prosody> xx.</speak>';
    ...
    it("SpeechList", function() {
      let converter = new ClovaSSML(breakAudios, 'ja');
      let ssml = '<speak>Hello!<break time="1.2s" /> ' +
    ...
    it("with xml declaration", function() {
      let converter = new ClovaSSML(breakAudios, 'ja');
      let ssml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
